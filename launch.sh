...
INT_WIFI=$1 # interface du point d'acces wifi
INT_NET=$2 # interface wlan ou eth0 ayant Internet
...
if [ $USER != "root" ]
then
    echo -e $RED"Vous devez etre root pour lancer ce progamme!"$NC
    exit 1
fi
...
if [ $? != 0 ]
then
    echo -e $RED"Erreur Fatale: Un probleme est survenue: Impossible de trouver la commande ifconfig!"$NC
    exit 1
fi
...
if [ $? != 0 ]
then
    echo -e $RED"Avertissement: macchanger non trouve. L'adresse mac ne sera pas modifiee!"$NC
    MACCHANGER="1"
fi
...
echo -e $blue"Demarrage et configuration de l'interface wifi $INT_WIF..."$NC
sudo ifconfig $INT_WIFI down
sleep 0.5
sudo ifconfig $INT_WIFI $IP netmask $MASK up
 
echo -e $blue"Demarrage daemon hostapd..."$NC
# start hostapd server (see /etc/hostapd/hostapd.conf)
sudo hostapd hostapd.conf &
sleep 1
 
echo -e $blue"Demarrage daemon dnsmasq... "$NC
# start dnsmasq server (see /etc/dnsmasq.conf) -7 /etc/dnsmasq.d
sudo dnsmasq -x /var/run/dnsmasq.pid -C dnsmasq.conf
sleep 1
 
echo -e $blue"Demarrage daemon dhcpd... "$NC
...
sudo dhcpd $DBG -f -pf /var/run/dhcp-server/dhcpd.pid -cf dhcpd.conf $INT_WIFI &
...
echo -e $blue"Activation iptables FORWARD & INPUT entre interface $NC$red$INT_WIFI$NC$blue & sous-reseau $NC$red$SUBNET$NC"
...
echo -e $cyan"Desactivation iptables NAT MASQUERADE...$NC$red$INT_NET$NC"$NC
...
echo -e $cyan"Desactivation iptables FORWARD & INPUT...$NC $red$INT_WIFI$NC$blue & $NC$red$SUBNET$NC"
# interface weak up!
sudo ifconfig $INT_WIFI down
